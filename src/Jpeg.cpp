...
int Jpeg::readImage(){
    try{
        ...
        header->printHeader();
        errHandle->printLog("Generating array with " + std::to_string((cinfo.num_components+1) * cinfo.image_height * cinfo.image_width) + " Bits storage\n");
        ...
        fclose(inFile);
        errHandle->printLog("Finished parsing jpeg\n");
        return ErrorHandler::errNoError;
    }
    catch(const std::exception& e){
        return ErrorHandler::errStdExcept;
    }
    catch(...){
        return ErrorHandler::errUnknown;
    }
}
